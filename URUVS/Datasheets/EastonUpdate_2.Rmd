...
2. When comparing models and whether or not to use month or time of day, they all performed really similarly, with models that include neither and models that just include time of day performing SLIGHTLY better than others (using month vs using both, which performed the worst). I kept the two slightly better models to test interactions between desa and site type (see table \@ref(tab:simpsontimemodels))

3. Comparing if desa and site type have an interaction barely changed the performance of the model again, so I just chose the model with no time or month component, and no interaction between desa and site type. OK SO MARKDOWN IS GIVING ME A TON OF GRIEF OVER SOME STUPID KNITTING ERROR WITH THE BRMS PACKAGE THAT DOESN'T APPEAR WHEN RUNNING THE CODE ON IT'S OWN AND I'VE BEEN TROUBLESHOOTING FOR LIKE THREE DAYS NOW AND I'M ABOUT TO THROW MY CHAIR ACROSS THE ROOM SO I'VE INCLUDED THE MODEL AND DATASHEET IN A SEPARATE R FILE THAT YOU SHOULD BE ABLE TO RUN BECAUSE I'M AT THE END OF MY ROPE HERE
...
```{r simpsontimemodels, echo = FALSE }
# library(brms)
# library(tidyverse)
# library(bayesplot)
# library(tidybayes)
...
# 
# 
# m_simpson <- add_criterion(m_simpson, "loo")#, moment_match = TRUE)
# m_simpson <- add_criterion(m_simpson, "loo", moment_match = TRUE)
# 
# #loo(m_simpson)
# 
...
# )
# 
# m3_simpson <- add_criterion(m3_simpson, "loo") #, moment_match = TRUE)
# 
# #loo(m3_simpson)
# 
# 
...
# 
# # Add LOO criterion to your fitted model
# m4_simpson <- add_criterion(m4_simpson, "loo")#, moment_match = TRUE)
# 
# #loo(m4_simpson)
# 
# 
...
#   mutate(signal_to_noise = abs(elpd_diff) / se_diff)
# 
# write.csv(loo1, 'EastonUpdateDocs/timeLoo.csv')
# 
# # knitr::kable(loo1, caption = "m: both time and month, m2: neither, m3: just time, m4: just month", format = "latex", booktabs = TRUE) %>%
# #   kable_styling(latex_options = "striped") %>%
# #   kable_styling(latex_options = "HOLD_position")

loo1 <- read.csv('EastonUpdateDocs/timeLoo.csv')
